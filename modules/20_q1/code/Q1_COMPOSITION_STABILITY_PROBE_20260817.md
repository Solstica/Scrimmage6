...
各 cell 的四块组成概率整体变化很小；最大单 cell 绝对偏离约 `0.00999`（RegionB × RealTimeInference，约 1.0 个百分点），其余多数更小。结合总体 `Cramer's V=0.02027` 和 p 值，未发现需要为 24 h 短期预测引入时变 `pi[r,k,t]` 的证据。
