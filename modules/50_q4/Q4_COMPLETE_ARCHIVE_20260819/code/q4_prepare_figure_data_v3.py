...
def write_recertification(out: Path, stage_metrics: pd.DataFrame) -> None:
    # 逐轮日志用于诊断；正式图只使用每个阶段的完成事件。
    stages = stage_metrics.loc[
        stage_metrics["事件"].astype(str).eq("stage_completed")
    ].copy()
    if stages.empty:
        raise ValueError("再认证阶段指标缺少 stage_completed 记录")
    stages = stages.sort_values(["阶段", "轮次"], kind="stable").reset_index(drop=True)
    stage_map = {"Cost再认证": "Cost", "Wait再认证": "Wait", "Latency再认证": "Latency"}
    stage_text = stages["阶段"].astype(str)
    stages["类别_阶段"] = stage_text.map(stage_map)
    stages.loc[stages["类别_阶段"].isna(), "类别_阶段"] = stage_text.str.extract(
        r"(Cost|Wait|Latency)", expand=False
    )
    stages["类别_阶段"] = stages["类别_阶段"].fillna(stage_text)
