...
def round_ui(row: dict, args) -> None:
    ...
    if event in ("added_region_benders_cuts", "added_region_benders_cuts_and_columns"):
        suffix = "，并新增完整域列" if event.endswith("and_columns") else ""
        print(
            f"[第{iteration}轮][1/5 LP与Benders] 未通过 | 最大区域违反={vmax:.6g} CNY "
            f"大于容差={args.benders_tol_cny:.6g}，主导区域={region}，cuts={row['Benders切数']}，"
            f"新增列={row['新增列数']}{suffix}；继续迭代。",
            flush=True,
        )
        return
...
def use_ext(spec: StageSpec) -> bool:
    # Cost 阶段也可以直接把 Renewable/BESS/Grid recourse 放进整数模型。
    # 这只在调用本模块的 stage solver 时生效，不改变 canonical full solver。
    return spec.objective == "cost" or spec.objective in ("wait", "latency") or spec.wait_cap is not None
...
def run_stage(spec: StageSpec, pool: ColumnPool, cuts: list[Cut], data, gpu_row: np.ndarray,
    ...
    for iteration in bar:
        ...
        if len(violated_regions):
            # 先加入当前违反区域的 Benders cuts，再立刻重解 RMP。
            # 不能直接拿“加 cut 之前”的旧对偶做 pricing：price_full_domain 内部
            # 通过 zip(cut_dual, cuts) 组合 cut 信号，新加入的 cuts 尚无对偶，
            # 会导致定价基于过时主问题，批量加入大量随后失效的列。
            ...
            cut_regions = np.asarray([cut.region for cut in cuts], dtype=np.int8)
            lp = solve_stage_lp(
                assign, resource, cm, rhs, const, objective, wait, cut_regions, spec
            )

            # 用“含最新 cuts 的 RMP”重新计算 recourse 与诊断量，再做完整域精确定价。
            load = facility_load(pool, data, lp["x"])
            energy = energy_lp(load, data)
            if not energy["success"]:
                raise StageStopped(spec.name + " 加割后 LP 能源子问题不可行：" + energy["message"])
            region_violation = energy["region_cost"] - lp["theta"]
            violation = float(energy["cost"] - lp["theta_total"])
            cap_violation = 0.0 if spec.cost_cap is None else float(energy["cost"] - spec.cost_cap)
            ...
            last_min_rc = min_rc
            if added:
                event = "added_region_benders_cuts_and_columns"
        else:
            additions, min_rc, _ = price_full_domain(
                pool, data, gpu_row, it_row, lp["resource_dual"], lp["cut_dual"], cuts,
                lp["assignment_dual"], spec, latency_raw, lp["wait_cap_dual"],
