...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{\ProjectRoot/modules/30_q2/figures/q2_flow.png}
\caption{问题二建模求解总体流程。}
\label{fig:q2-flow}
\end{figure}
...
三类任务的时空柔性存在显著差异，图\ref{fig:q2-flexibility} 展示了各任务类型在空间可行区域数 $|R_i|$、真正可调时间自由度和 GPU-hour 三个维度的分布。其中 slack 定义为
\begin{equation}
\text{Slack}_i = \min(d_i, 2406) - a_i - p_i,
\label{eq:q2-slack}
\end{equation}
而真正可调时间自由度对 RT 严格为 0（$s_i = a_i$ 是硬约束），仅对 Batch 和 Training 有意义。

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{\ProjectRoot/modules/30_q2/figures/Q2-1 任务柔性三面板.png}
\caption{三类任务的时空柔性与资源重量对比。(a) 空间合法区域数分布；(b) 真正可调时间自由度，RT 因 $s_i = a_i$ 强制即时开工故自由度为 0；(c) GPU-hour 中位数分布。数据表明：空间越受限、时间越刚性、资源重量越大的任务越应优先调度。}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.9\textwidth]{\ProjectRoot/modules/30_q2/figures/Q2-2 基准能源时空错配及低碳属性.png}
\caption{基准能源时空错配及弃电的低碳属性。(a) 弃电与供负荷购电并存统计：14\,400 个 region-hour 中 13\,230 个（91.9\%）同时存在 $\mathrm{Curtailment}>0$ 和 $\mathrm{GridToLoad}>0$；累计弃电量约为基准 AI 设施侧能量的 8.15 倍。(b) 六区域弃电与碳强度、可用新能源的 Pearson 相关系数。弃电与碳强度均为显著负相关（$-0.690$ 至 $-0.849$），与可用新能源均为强正相关（$0.925$ 至 $0.995$）。该图说明 Q2 的调度价值来源是算力负荷与绿色能源的时空错配，且优先吸收弃电具有数据驱动的低碳属性。}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.9\textwidth]{\ProjectRoot/modules/30_q2/figures/Q2-3 分段能源边际响应.png}
\caption{设施负荷增量与能源响应的分段关系。横轴为设施负荷增量 $\Delta L$，纵轴分别展示弃电功率变化 $\Delta\text{Curtailment}$（橙色）和电网购电功率变化 $\Delta\text{GridPurchase}$（蓝色）。分段机制为：I 区 $0 \le \Delta L \le \text{Curtailment}_0$，新增负荷全部由原有弃电吸收，$\Delta G = 0$；II 区 $\Delta L > \text{Curtailment}_0$，弃电耗尽后触发新增购电；III 区 $\Delta L < 0$ 且 $G_0 + \Delta G > 0$，负荷下降先减少原有供负荷购电；IV 区 $G_0 + \Delta G = 0$，购电降为零后继续降负荷会增加弃电。该分段机制是增量能量平衡模型的核心。}
...
综合任务时空柔性分析与基准能源错配诊断，问题二的完整数学模型可归纳为如下优化问题：

\begin{equation}
\left\{
\begin{aligned}
& \text{决策变量：} && x_{i,r,s} \in \{0,1\}, && \forall i,\; r\in R_i,\; s\in T_i; \\
& \text{主目标：} && \min \; \sum_{r,t} \bigl[ \mathrm{GridPurchase}[r,t] \cdot \mathrm{Price}[r,t] - \mathrm{GridSell}_0[r,t] \cdot \mathrm{SellPrice}[r,t] \bigr]; \\
& \text{约束条件：} \\
& && \sum_{r\in R_i}\sum_{s\in T_i} x_{i,r,s} = 1, && \forall i; \\
& && \sum_{i,s} g_i \cdot \omega(i,s,t) \cdot x_{i,r,s} \le \mathrm{AvailableGPU}[r], && \forall r,t; \\
& && P_{\mathrm{IT}}[r,t](\mathbf{x}) \le \mathrm{MaxITPower}[r], && \forall r,t; \\
& && L[r,t](\mathbf{x}) \le \mathrm{MaxFacilityPower}[r], && \forall r,t; \\
& && 0 \le \mathrm{GridPurchase}[r,t] \le \mathrm{MaxGridImport}[r], && \forall r,t; \\
& && \sum_{s} x_{i,r,s} \cdot \mathrm{NetworkLatency}[o_i, r] \le L_i, && \forall i; \\
& && s_i + p_i \le \min(d_i, 2406),\quad s_i = a_i \ (k_i = \mathrm{RT}), && \forall i; \\
& \text{增量能源规则：} \\
& && \Delta G[r,t] = 
\begin{cases}
...
-\min(-\Delta L[r,t], G_0[r,t]), & \Delta L[r,t] < 0;
\end{cases} \\
& \text{边界与口径：} && \text{固定基准储能与购售电策略；调度敏感时域 } [0,2405]; \\
& \text{评价指标：} && \mathrm{Carbon},\; \eta_R \text{（平行评价，不纳入主目标）}; \\
& \text{求解策略：} && \text{外层重复完整扫描至 } (J_{k-1}-J_k)/|J_{k-1}| < 10^{-3}.
\end{aligned}
\right.
\label{eq:q2-summary}
\end{equation}

其中，$\mathrm{GridPurchase}[r,t] = \mathrm{GridPurchase}_0[r,t] + \Delta G[r,t]$，$L[r,t](\mathbf{x}) = \mathrm{PUE}[r] \cdot (\mathrm{NonAI\_IT\_Load}[r,t] + P_{\mathrm{AI}}[r,t](\mathbf{x}))$，$\Delta L[r,t] = L[r,t](\mathbf{x}) - L_0[r,t]$，$G_0[r,t]$为基准供负荷购电功率。模型采用Cost-primary为主目标，Carbon-primary作为对照端点，所有硬约束通过审计门禁后输出最终调度方案。
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{\ProjectRoot/modules/30_q2/figures/Q2-2 收敛与初始化敏感性.png}
\caption{SFETA 重复完整扫描收敛轨迹与初始化敏感性。(a) Cost-primary 端点：reference-start（深蓝实线）与 previous-start（橙色虚线）的逐轮成本变化。reference-start 第 3 遍达到约 $1.555\times10^9$ CNY 并满足收敛容差；previous-start 第 4 遍仍收敛到更高成本，揭示初始化路径依赖。(b) Carbon-primary 端点（绿色）与 Cost-primary 端点（深蓝）的碳排收敛轨迹。两条轨迹总体同向，但 Carbon-primary 收敛到更低碳排，验证了 Cost/Carbon 双端点对照的必要性。}
...
\caption{代表性任务调度甘特图（9 个典型任务，时刻 2379--2406）。按实时推理（上）、批量推理（中）、AI 训练（下）三类排列。甘特条为执行区间（StartHour $\to$ FinishHour），空心三角标记到达时刻 ArrivalHour。RT 三条任务均到达即开工，同时包含原区执行和跨区迁移；Batch 展示短延迟、原区即时和终端收尾三种状态；AI 训练展示跨区高负载、终端高负载和跨区短延迟。该图仅用于直观解释调度结构，不代表全部 50\,000 个任务的排程特征。}
\label{fig:q2-gantt}
\end{figure}
...
\caption{来源区域到执行区域的迁移任务数热图。单元格数值为对应迁移的任务数；纵轴为来源区域，横轴为执行区域。对角线单元格表示任务保持来源区域不迁移，非对角线单元格表示跨区域迁移。主对角线仍占一定比例，不强制均匀化迁移；真实迁移方向由实际约束和能源信号共同决定，不预先指定某区域为绿色承接中心。}
...
\caption{Cost--Carbon 端点关系散点图。横轴为运行成本（CNY），纵轴为碳排放（tCO\textsubscript{2}），包含附件基准（灰色叉号）、Cost-primary reference-start（深蓝圆点）、Carbon-primary reference-start（绿色菱形）和 Cost-primary previous-start（橙色方点）四个端点。两个 canonical 端点（Cost-primary 和 Carbon-primary）高度同向但不完全重合，且均显著优于基准；previous-start 端点同时偏向更高成本和更高碳排，体现路径依赖。Carbon-primary 相比 Cost-primary 的成本代价约 $0.18\%$，Cost-primary 相比 Carbon-primary 的碳代价约 $0.62\%$。仅展示四个代表点，不称 Pareto frontier。}
